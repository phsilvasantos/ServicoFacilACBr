...
uses
  ...
  pnfsConversao, ACBrDelphiZXingQRCode, RLPreviewForm;
...
begin
  inherited;
  if CodCidadeToCidade(StrToIntDef(fpNFSe.Servico.CodigoMunicipio, 0)) <> EmptyStr then
  begin
    Tamanho := Length(CodCidadeToCidade(StrToIntDef(fpNFSe.Servico.CodigoMunicipio, 0)));
    NomeCidade := Copy(CodCidadeToCidade(StrToIntDef(fpNFSe.Servico.CodigoMunicipio, 0)), 1, Tamanho - 2);
  end;
  RLNFSe.Title := 'NFS-e: ' + fpNFSe.Numero + ' ' + NomeCidade;
