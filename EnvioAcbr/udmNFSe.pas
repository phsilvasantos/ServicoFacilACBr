...
begin
   ...
   {$ENDIF}
    ...
    ACBrNFSe1.DANFSe.Prefeitura := 'ESTADO DO RIO GRANDE DO SUL;;' +
                                   'PREFEITURA MUNICIPAL ' + fDMCadNotaServico.cdsFilialCIDADE.AsString + ';;' +
                                   'SECRETARIA MUNICIPAL DA FAZENDA ';

    ACBrNFSe1.DANFSE.Logo := ExtractFilePath(Application.ExeName) + 'LogoPrefeitura.bmp';
  ...
  ACBrNFSeDANFSeRL1.PathPDF := fDMCadNotaServico.cdsParametrosENDPDFNFSE.AsString;
...
//      NATUREZA_PADRAO := fDMCadNotaServico.cdsNotaServico_ImpCOD_NATUREZA.AsString;
      NATUREZA_PADRAO := fDMCadNotaServico.cdsNotaServico_ImpCOD_NATUREZA_PROV.AsString;
...
//      end;
     ...
     // Informar para Saatri
      Servico.CodigoPais    := StrToIntDef(Monta_Numero(fDMCadNotaServico.cdsNotaServico_ImpCODPAIS_CLI.AsString,0),1058);
...
//  GerarNFSe;
  ...
  begin
    ...
    begin
      ...
      ACBrNFSe1.NotasFiscais.Items[vCont].NFSe.Competencia :=
           fDMCadNotaServico.cdsNotaServico_ImpAno_REF.AsString + Formatar_Campo(fDMCadNotaServico.cdsNotaServico_ImpMES_REF.AsString,2);
    end;
...
var
  vAux, vProtocolo, Caminho, vCodigoVerificacao, vNumero: string;
...
begin
  ...
  if dmNFSe.OffLine then
  begin
    vNumeroLote := fDMCadNotaServico.fnc_Buscar_NumLote(fDMCadNotaServico.cdsFilialID.AsInteger);
    fDMCadNotaServico.prc_Abrir_Impressao(fID_NOTA);
    prc_Abrir_NotaServico_Comunicacao(fID_NOTA);
    ConfigurarComponente;
    AlimentaComponente;
  end;

//  qNotaServico_Comunicacao.Close;
//  qNotaServico_Comunicacao.ParamByName('ID_NOTASERVICO').Value := Value;
...
begin
  ...
  begin
    ...
    fDMCadNotaServico.prc_Abrir_Comunicacao(fDMCadNotaServico.cdsNotaServico_ConsultaID.AsInteger);
...
//    vDiscriminacao := vDiscriminacao + '  Valor RS ' + FormatFloat('###,###,##0.00',fDMCadNotaServico.cdsNotaServico_Imp_ItensVLR_TOTAL.AsFloat) + ')';
    ...
    begin
      ...
      begin
        if trim(vTexto1) = '' then
          ...
          vTexto1 := vTexto1 + '   -  ';
        vTexto1 := vTexto1 + fDMCadNotaServico.cdsNotaServico_Imp_ParcDTVENCIMENTO.AsString + ' RS ' +
  ...
  if fDMCadNotaServico.qParametros_SerIMPRIMIR_IMPOSTOS.AsString = 'S' then
  begin
...
//                      fDMCadNotaServico.cdsNotaServico_ImpANO_REF.AsString +   ')';
  ...
  begin
    ...
    begin
      vDiscriminacao := vDiscriminacao + '(Vlr. aproximado total de tributos federais, estaduais, e municipais cfe. disposto na lei 12.741/12. RS ' +
    ...
    begin
      ...
      vTexto := '(Vlr.aprox. tributos RS ' + FormatFloat('###,###,##0.00',fDMCadNotaServico.cdsNotaServico_ImpVLR_TRIBUTO_FEDERAL.AsFloat);
      ...
      vTexto := vTexto + ' RS ' + FormatFloat('###,###,##0.00',fDMCadNotaServico.cdsNotaServico_ImpVLR_TRIBUTO_ESTADUAL.AsFloat);
      if fDMCadNotaServico.cdsParametrosIMP_PERC_TRIB_SERVICO.AsString = 'S' then
        vTexto := vTexto + ' %' + FormatFloat('0.00',vPercAux_Estadual);
      vTexto := vTexto + ' Estadual e RS ' + FormatFloat('###,###,##0.00',fDMCadNotaServico.cdsNotaServico_ImpVLR_TRIBUTO_MUNICIPAL.AsFloat);
      if fDMCadNotaServico.cdsParametrosIMP_PERC_TRIB_SERVICO.AsString = 'S' then
        vTexto := vTexto + ' %' + FormatFloat('0.00',vPercAux_Municipal);
      vTexto := vTexto + ' Municipal  RS ' + FormatFloat('###,###,##0.00',fDMCadNotaServico.cdsNotaServico_ImpVLR_TRIBUTO.AsFloat);
      if fDMCadNotaServico.cdsParametrosIMP_PERC_TRIB_SERVICO.AsString = 'S' then
...
begin
  ...
  begin
    ...
    fDMCadNotaServico.prc_Abrir_Comunicacao(fDMCadNotaServico.cdsNotaServico_ConsultaID.AsInteger);
    if fDMCadNotaServico.cdsNotaServico_Comunicacao.Locate('TIPO',1,[loCaseInsensitive]) then
      fDMCadNotaServico.cdsNotaServico_Comunicacao.Edit
    else
    begin
      fDMCadNotaServico.cdsNotaServico_Comunicacao.Insert;
