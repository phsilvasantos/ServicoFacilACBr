...
uses DmdDatabase, rsDBUtils, UMenu, uUtilPadrao, USel_Pessoa, URelDanfe_NFSe, URelDanfe_NFSe_SL, Math, uUtilCliente,
  ...
  begin
    ...
    try
      ...
      if chkOffLine.Checked then
        fDMNFSe.GerarOffline(fDMCadNotaServico.cdsNotaServico_ImpID.AsInteger)
      else
        fdmNFSe.Enviar_Nfse;
    finally
      FreeAndNil(fDMNFSe);
...
procedure TfrmCadNotaServico_acbr.btnConsultar_NFSeClick(Sender: TObject);
begin
  if not(fDMCadNotaServico.cdsNotaServico_Consulta.Active) and (fDMCadNotaServico.cdsNotaServico_ConsultaID.AsInteger <= 0) then
    exit;

  fDMCadNotaServico.prc_Abrir_Impressao(fDMCadNotaServico.cdsNotaServico_ConsultaID.AsInteger);
  if not fDMCadNotaServico.cdsServico.Active then
    fDMCadNotaServico.prc_Abrir_Servico;
